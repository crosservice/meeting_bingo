      ...
      <div className="mx-auto max-w-2xl">
        ...
        {games.length === 0 ? (
          ...
          <ul className="space-y-2">
            {games.map((g) => (
              <li key={g.id} className="flex items-center justify-between rounded border border-gray-200 p-3">
                <div>
                  <span className="text-sm font-medium">Game</span>
                  <span className={`ml-2 text-xs px-2 py-0.5 rounded ${
                    g.status === 'active' ? 'bg-green-100 text-green-700' :
                    g.status === 'won' ? 'bg-yellow-100 text-yellow-700' :
                    g.status === 'draft' ? 'bg-gray-100 text-gray-600' :
