      ...
      <div className="mx-auto max-w-2xl">
          ...
          <div className="mb-4 flex gap-2 flex-wrap">
            {sets.map((s) => (
              <button key={s.id} onClick={() => setActiveSetId(s.id)}
                className={`rounded px-3 py-1 text-sm ${activeSetId === s.id ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
                {s.name}
              </button>
            ))}
          </div>
        ...
        {activeSetId && (
          <>
            ...
            {warning && <div className="mb-3 rounded bg-amber-50 p-2 text-sm text-amber-700">{warning}</div>}

            <p className="mb-2 text-sm text-gray-500">{phrases.filter((p) => p.is_active).length} active phrases (need 24-25 for a 5x5 board)</p>
